...
- Variables/stdlib: `set`, `unset`, `option`, `mark_as_advanced`, `separate_arguments`, `list`, `string`, `math`, `message`, `load_cache`, legacy compatibility helpers (`remove`, `variable_watch`)
...
- Filesystem and packaging: `file`, `aux_source_directory`, `configure_file`, `find_package`, `install`, legacy install wrappers (`install_files`, `install_programs`, `install_targets`), legacy file wrappers (`make_directory`, `write_file`), `export`, `cmake_file_api`, CPack subset
- Tests/custom/other: `enable_testing`, `add_test`, `create_test_sourcelist`, `ctest_*` command family, `include_external_msproject`, legacy compatibility family (`export_library_dependencies`, `load_command`, `output_required_files`, `qt_wrap_cpp`, `qt_wrap_ui`, `fltk_wrap_ui`, `subdir_depends`, `subdirs`, `use_mangled_mesa`, `utility_source`, `variable_requires`), `add_custom_target`, `add_custom_command`, `execute_process`, `exec_program`, `try_compile`, `try_run`
