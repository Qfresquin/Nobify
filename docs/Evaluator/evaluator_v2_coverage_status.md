...
- This matrix is authoritative for the `119` dispatcher-registered built-in commands exposed by `src_v2/evaluator/eval_command_caps.c`.
- It does not by itself represent the entire CMake command universe.
- The broader audit scope is tracked in `evaluator_v2_full_audit.md`: `131` scoped documented entry points (`128` from `cmake-commands(7)` + `3` `CPackComponent` module commands), of which `131` are currently implemented (`119` registry-backed + `12` structural parser/evaluator commands) and `0` remain missing.
...
| `aux_source_directory` | `FULL` | `NOOP_WARN` | The documented one-line signature is modeled with a deterministic non-recursive directory scan over evaluator-supported source extensions (`.c`, `.cc`, `.cpp`, `.cxx`, `.m`, `.mm`), returning a sorted semicolon list. | - |
...
| `cmake_file_api` | `PARTIAL` | `NOOP_WARN` | The evaluator implements the `QUERY API_VERSION 1` command surface as query-registration metadata only, recording supported object requests (`CODEMODEL`, `CACHE`, `CMAKEFILES`, `TOOLCHAINS`) in evaluator variables. It does not generate a reply tree, invoke CMake, or model non-`QUERY` subcommands. | `Medium` |
...
| `create_test_sourcelist` | `FULL` | `NOOP_WARN` | The documented signature is modeled by generating a deterministic driver source in the current binary directory, honoring `EXTRA_INCLUDE`, `FUNCTION`, and the `CMAKE_TESTDRIVER_BEFORE_TESTMAIN` / `CMAKE_TESTDRIVER_AFTER_TESTMAIN` hooks, while publishing the resulting source list variable. | - |
...
| `export_library_dependencies` | `PARTIAL` | `NOOP_WARN` | The evaluator accepts the legacy command and records deterministic compatibility metadata, but it does not synthesize the historical dependency file content. Modern `install(EXPORT)` / `export()` flows remain the authoritative implementation paths. | `Low` |
| `export` | `PARTIAL` | `NOOP_WARN` | The evaluator implements build-tree export metadata for `export(TARGETS ...)` and `export(EXPORT ...)`, including deterministic metadata-file output and tracked install-export resolution. Package-registry/setup forms, `CXX_MODULES_DIRECTORY`, and full native export-file semantics remain out of scope. | `Medium` |
...
| `fltk_wrap_ui` | `PARTIAL` | `NOOP_WARN` | The evaluator parses the legacy wrapper surface and publishes deterministic generated file names into `<target>_FLTK_UI_SRCS`, but it does not invoke FLTK tooling. | `Low` |
...
| `include_external_msproject` | `PARTIAL` | `NOOP_WARN` | The documented argument order is parsed and persisted as evaluator metadata, and the command creates a target stub so dependency declarations can refer to the imported external project. Visual Studio solution/project generation is not modeled. | `Medium` |
...
| `install_files` | `PARTIAL` | `NOOP_WARN` | The evaluator accepts the legacy compatibility command and emits install rules for the explicit file-list form used in covered flow. Regex auto-discovery and extension-rewrite edge cases remain intentionally narrower than historical CMake behavior. | `Low` |
| `install_programs` | `PARTIAL` | `NOOP_WARN` | The evaluator accepts the legacy compatibility command and emits deterministic install-program rules for explicit file lists. Historical regex/discovery forms are not modeled. | `Low` |
| `install_targets` | `PARTIAL` | `NOOP_WARN` | The evaluator accepts the covered legacy compatibility form and emits target install rules for declared targets, including the narrow `RUNTIME_DIRECTORY` parse path. The broader historical signature surface remains narrower than full CMake. | `Low` |
...
| `load_command` | `PARTIAL` | `NOOP_WARN` | The evaluator accepts the obsolete command and records deterministic compatibility metadata only. It does not load native command modules. | `Low` |
| `load_cache` | `PARTIAL` | `NOOP_WARN` | The evaluator implements the documented `READ_WITH_PREFIX` flow plus the legacy import form with `EXCLUDE` and `INCLUDE_INTERNALS`, reading canonical `key:type=value` entries from `CMakeCache.txt`. Parsing is intentionally narrower than historical CMake edge cases: malformed/non-canonical lines are skipped with warnings. | `Medium` |
| `make_directory` | `FULL` | `NOOP_WARN` | The evaluator creates the requested directory path, recursively creating missing parents in the same way as the documented legacy compatibility command. | - |
...
| `output_required_files` | `PARTIAL` | `NOOP_WARN` | The evaluator accepts the obsolete command and records deterministic compatibility metadata only. It does not attempt historical dependency scanning. | `Low` |
...
| `qt_wrap_cpp` | `PARTIAL` | `NOOP_WARN` | The evaluator parses the deprecated Qt 3 compatibility surface and publishes deterministic `moc_<stem>.cxx` outputs into the requested variable. It does not invoke `moc`. | `Low` |
| `qt_wrap_ui` | `PARTIAL` | `NOOP_WARN` | The evaluator parses the deprecated compatibility surface and publishes deterministic generated header/source names into the requested output variables. It does not invoke Qt UI tooling. | `Low` |
| `remove` | `PARTIAL` | `NOOP_WARN` | The evaluator accepts the legacy list-variable form and routes it through an evaluator-local compatibility path, but the broader historical edge-case surface of the obsolete command is not fully validated in this batch. | `Low` |
...
| `subdir_depends` | `PARTIAL` | `NOOP_WARN` | The evaluator accepts the deprecated command and records deterministic compatibility metadata only. It does not alter directory scheduling semantics. | `Low` |
| `subdirs` | `PARTIAL` | `NOOP_WARN` | The evaluator accepts the deprecated command and records deterministic compatibility metadata only. Projects should use `add_subdirectory()` for real execution semantics. | `Low` |
...
| `use_mangled_mesa` | `PARTIAL` | `NOOP_WARN` | The evaluator accepts the deprecated command and records deterministic compatibility metadata only. It does not model historical Mesa path mangling. | `Low` |
| `utility_source` | `PARTIAL` | `NOOP_WARN` | The evaluator accepts the deprecated command and records deterministic compatibility metadata only. It does not generate additional utility-source behavior beyond the metadata trace. | `Low` |
| `variable_requires` | `PARTIAL` | `NOOP_WARN` | The evaluator accepts the deprecated command and records deterministic compatibility metadata only. It does not enforce the historical variable gate semantics in this batch. | `Low` |
| `variable_watch` | `PARTIAL` | `NOOP_WARN` | The evaluator registers watched non-cache variables and records deterministic notifications on `set()` / `unset()`, publishing the last watched mutation in metadata. It does not execute callback commands or model read-access callbacks. | `Medium` |
| `write_file` | `PARTIAL` | `NOOP_WARN` | The evaluator writes or appends file content through the legacy compatibility surface, including multiple content arguments and trailing `APPEND`. It does not model additional historical edge cases outside that covered form. | `Low` |
...
The authoritative `FULL` set is the subset of rows in the command matrix above marked `FULL` (currently `67` commands).

Commands currently documented as `PARTIAL` (`52`): `build_command`, `build_name`, `cmake_host_system_information`, `cmake_file_api`, `cmake_language`, `ctest_build`, `ctest_configure`, `ctest_coverage`, `ctest_empty_binary_directory`, `ctest_memcheck`, `ctest_read_custom_files`, `ctest_run_script`, `ctest_sleep`, `ctest_start`, `ctest_submit`, `ctest_test`, `ctest_update`, `ctest_upload`, `exec_program`, `export`, `export_library_dependencies`, `fltk_wrap_ui`, `get_cmake_property`, `get_directory_property`, `get_property`, `get_source_file_property`, `get_target_property`, `get_test_property`, `include_external_msproject`, `install_files`, `install_programs`, `install_targets`, `load_cache`, `load_command`, `output_required_files`, `qt_wrap_cpp`, `qt_wrap_ui`, `remove`, `remove_definitions`, `separate_arguments`, `source_group`, `subdir_depends`, `subdirs`, `target_compile_features`, `target_precompile_headers`, `target_sources`, `try_run`, `use_mangled_mesa`, `utility_source`, `variable_requires`, `variable_watch`, `write_file`.

## 11.1 Full-scope summary

- Dispatcher-backed built-ins tracked by this document: `119`.
...
- Missing commands from that broader scope: `0`.
