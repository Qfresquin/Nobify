...
| Target declaration, kind identity, alias/imported identity | `target` | `partial` | Basic target identity is solid, but broader imported/alias edge cases still interact with later slices. | New target-family work fits into the existing target record shape without structural redesign. | `moderate` |
| Source membership, file sets, and source-file properties | `target`, `build_step` | `partial` | Complex source metadata and file-set interactions still tend to escape the current "just sources" model. | Artifact-relevant source metadata lands canonically and codegen no longer needs new source-shape fields for common project classes. | `moderate` |
