...
| `add_test` | `eval_test.c` | complete | `legacy_structural` | `TEST`, `TRACE` | `EVENT_TRACE_COMMAND_BEGIN`, `EVENT_TRACE_COMMAND_END` | high | Test payload moves later. |
| `install` | `eval_install.c` | complete | `legacy_structural` | `INSTALL`, `TRACE` | `EVENT_TRACE_COMMAND_BEGIN`, `EVENT_TRACE_COMMAND_END` | high | Install-rule payload moves later. |
| `find_package` | `eval_package.c` | complete | `legacy_structural` / `local_only` | `PACKAGE`, `TRACE`, `DIAG` | `EVENT_TRACE_COMMAND_BEGIN`, `EVENT_DIAG`, `EVENT_TRACE_COMMAND_END` | high | Semantic package result events come after trace/diag base is stable. |
