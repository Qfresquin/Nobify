...
| `block` | `eval_flow.c` | complete | `local_only` | `SCOPE`, `FLOW` | `EVENT_SCOPE_PUSH`, `EVENT_SCOPE_POP`, `EVENT_FLOW_RETURN` | high | Scope semantics already exist; block-specific flow payload can still expand later. |
| `function` | `evaluator.c` | complete | `local_only` | `SCOPE` | `EVENT_SCOPE_PUSH`, `EVENT_SCOPE_POP` | high | Call entry/exit is represented only by scope semantics until a dedicated call family exists. |
| `macro` | `evaluator.c` | complete | `local_only` | `SCOPE` | `EVENT_SCOPE_PUSH`, `EVENT_SCOPE_POP` | high | Macro binding effects later map to `VAR`; no trace fallback remains. |
