...
static Eval_Result eval_foreach(Evaluator_Context *ctx, const Node *node) {
    ...
    if (idx < arena_arr_len(a) && eval_sv_eq_ci_lit(a[idx], "RANGE")) {
        ...
        if (arena_arr_len(a) - idx == 1) {
            if (!sv_parse_long(a[idx], &stop)) return eval_result_fatal();
        } else {
            if (!sv_parse_long(a[idx], &start)) return eval_result_fatal();
            if (!sv_parse_long(a[idx + 1], &stop)) return eval_result_fatal();
        ...
        for (long v = start;; v += step) {
            ...
            if (!sv_list_push(ctx->arena, &items, nob_sv_from_cstr(buf))) {
                (void)ctx_oom(ctx);
                return eval_result_fatal();
            }
        ...
        if (idx < arena_arr_len(a) && eval_sv_eq_ci_lit(a[idx], "ITEMS")) {
            ...
            for (size_t row = 0; row < max_len; row++) {
                ...
                nob_sb_free(sb);
                if (!sv_list_push(ctx->arena, &items, zipped)) {
                    (void)ctx_oom(ctx);
                    return eval_result_fatal();
                }
            }
    ...
    for (size_t ii = 0; ii < items_count; ii++) {
        ...
        Eval_Result body = eval_node_list(ctx, &node->as.foreach_stmt.body);
...
static Eval_Result eval_while(Evaluator_Context *ctx, const Node *node) {
    ...
    for (size_t iter = 0; iter < kMaxIter; iter++) {
        ...
        iter_count++;
        Eval_Result body = eval_node_list(ctx, &node->as.while_stmt.body);
        if (eval_result_is_fatal(body)) {
            ctx->loop_depth--;
            return body;
        }
...
bool eval_user_cmd_invoke(Evaluator_Context *ctx, String_View name, const SV_List *args, Cmake_Event_Origin origin) {
    ...
    body_result = eval_node_list(ctx, &cmd->body);
...
Eval_Result eval_execute_file(Evaluator_Context *ctx,
    ...
    result = eval_node_list(ctx, &new_ast);
...
Eval_Result evaluator_run(Evaluator_Context *ctx, Ast_Root ast) {
    ...
    size_t entered_file_depth = ++ctx->file_eval_depth;
    eval_report_reset(ctx);
    Eval_Result result = eval_node_list(ctx, &ast);
    if (!eval_result_is_fatal(result) && !eval_should_stop(ctx)) {
        if (!eval_defer_flush_current_directory(ctx)) result = eval_result_fatal();
    }
    if (!eval_result_is_fatal(result) && !eval_should_stop(ctx)) {
