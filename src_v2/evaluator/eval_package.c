...
static void find_package_push_prefix_list_variants(EvalExecContext *ctx,
    ...
    if (!ctx || !items || !io_count || prefixes_csv.count == 0) return;

    SV_List prefixes = NULL;
    if (!find_package_split_semicolon_temp(ctx, prefixes_csv, &prefixes)) return;
    for (size_t i = 0; i < arena_arr_len(prefixes); i++) {
        if (prefixes[i].count == 0) continue;
        find_package_push_prefix_variants(ctx, items, io_count, cap, prefixes[i]);
    }
}
...
static void find_package_push_env_prefix_list_variants(EvalExecContext *ctx,
    ...
    while (p <= end) {
        ...
        String_View item = nob_sv_from_parts(p, (size_t)(q - p));
        if (item.count > 0) {
            find_package_push_prefix_variants(ctx, items, io_count, cap, item);
        }
        if (q >= end) break;
        p = q + 1;
    }
...
static bool find_package_try_config(EvalExecContext *ctx,
    ...
    find_package_push_prefix_list_variants(ctx,
                                           merged_prefixes,
                                           &merged_count,
    ...
    if (!no_default_path) {
        ...
        if (!no_cmake_environment_path) {
            find_package_push_env_prefix_list_variants(ctx,
                                                       merged_prefixes,
                                                       &merged_count,
                                                       NOB_ARRAY_LEN(merged_prefixes),
                                                       "CMAKE_PREFIX_PATH");
...
static const Eval_Opt_Spec k_find_pkg_specs[] = {
    EVAL_OPT_SPEC(FIND_PKG_OPT_REQUIRED, "REQUIRED", EVAL_OPT_FLAG),
    EVAL_OPT_SPEC(FIND_PKG_OPT_QUIET, "QUIET", EVAL_OPT_FLAG),
    EVAL_OPT_SPEC(FIND_PKG_OPT_MODULE, "MODULE", EVAL_OPT_FLAG),
    EVAL_OPT_SPEC(FIND_PKG_OPT_CONFIG, "CONFIG", EVAL_OPT_FLAG),
    EVAL_OPT_SPEC(FIND_PKG_OPT_NO_MODULE, "NO_MODULE", EVAL_OPT_FLAG),
    EVAL_OPT_SPEC(FIND_PKG_OPT_EXACT, "EXACT", EVAL_OPT_FLAG),
    EVAL_OPT_SPEC(FIND_PKG_OPT_NAMES, "NAMES", EVAL_OPT_MULTI),
    EVAL_OPT_SPEC(FIND_PKG_OPT_CONFIGS, "CONFIGS", EVAL_OPT_MULTI),
    EVAL_OPT_SPEC(FIND_PKG_OPT_PATH_SUFFIXES, "PATH_SUFFIXES", EVAL_OPT_MULTI),
    EVAL_OPT_SPEC(FIND_PKG_OPT_COMPONENTS, "COMPONENTS", EVAL_OPT_MULTI),
    EVAL_OPT_SPEC(FIND_PKG_OPT_OPTIONAL_COMPONENTS, "OPTIONAL_COMPONENTS", EVAL_OPT_MULTI),
    EVAL_OPT_SPEC(FIND_PKG_OPT_HINTS, "HINTS", EVAL_OPT_MULTI),
    EVAL_OPT_SPEC(FIND_PKG_OPT_PATHS, "PATHS", EVAL_OPT_MULTI),
    EVAL_OPT_SPEC(FIND_PKG_OPT_NO_DEFAULT_PATH, "NO_DEFAULT_PATH", EVAL_OPT_FLAG),
    EVAL_OPT_SPEC(FIND_PKG_OPT_NO_PACKAGE_ROOT_PATH, "NO_PACKAGE_ROOT_PATH", EVAL_OPT_FLAG),
    EVAL_OPT_SPEC(FIND_PKG_OPT_NO_CMAKE_PATH, "NO_CMAKE_PATH", EVAL_OPT_FLAG),
    EVAL_OPT_SPEC(FIND_PKG_OPT_NO_CMAKE_ENVIRONMENT_PATH, "NO_CMAKE_ENVIRONMENT_PATH", EVAL_OPT_FLAG),
    EVAL_OPT_SPEC(FIND_PKG_OPT_NO_SYSTEM_ENVIRONMENT_PATH, "NO_SYSTEM_ENVIRONMENT_PATH", EVAL_OPT_FLAG),
    EVAL_OPT_SPEC(FIND_PKG_OPT_NO_CMAKE_SYSTEM_PATH, "NO_CMAKE_SYSTEM_PATH", EVAL_OPT_FLAG),
    EVAL_OPT_SPEC(FIND_PKG_OPT_NO_CMAKE_INSTALL_PREFIX, "NO_CMAKE_INSTALL_PREFIX", EVAL_OPT_FLAG),
    EVAL_OPT_SPEC(FIND_PKG_OPT_NO_CMAKE_PACKAGE_REGISTRY, "NO_CMAKE_PACKAGE_REGISTRY", EVAL_OPT_FLAG),
    EVAL_OPT_SPEC(FIND_PKG_OPT_NO_CMAKE_SYSTEM_PACKAGE_REGISTRY, "NO_CMAKE_SYSTEM_PACKAGE_REGISTRY", EVAL_OPT_FLAG),
    EVAL_OPT_SPEC(FIND_PKG_OPT_REGISTRY_VIEW, "REGISTRY_VIEW", EVAL_OPT_OPTIONAL_SINGLE),
    EVAL_OPT_SPEC(FIND_PKG_OPT_GLOBAL, "GLOBAL", EVAL_OPT_FLAG),
    EVAL_OPT_SPEC(FIND_PKG_OPT_NO_POLICY_SCOPE, "NO_POLICY_SCOPE", EVAL_OPT_FLAG),
    EVAL_OPT_SPEC(FIND_PKG_OPT_BYPASS_PROVIDER, "BYPASS_PROVIDER", EVAL_OPT_FLAG),
    EVAL_OPT_SPEC(FIND_PKG_OPT_UNWIND_INCLUDE, "UNWIND_INCLUDE", EVAL_OPT_FLAG),
};
...
static bool find_package_execute_resolved_artifacts(EvalExecContext *ctx,
                                                    ...
                                                    bool *io_found,
                                                    String_View found_path) {
    if (!ctx || !opt || !io_found) return false;
    if (!*io_found || found_path.count == 0) return true;

    find_package_publish_path_vars(ctx, opt, found_path);
...
static bool find_package_invoke_dependency_provider(EvalExecContext *ctx,
                                                    ...
                                                    bool *out_found,
                                                    String_View *out_found_path) {
    if (!ctx || !node || !resolved_args || !out_found || !out_found_path) return false;
    *out_found = false;
    *out_found_path = nob_sv_from_cstr("");
    ...
    String_View config_key = svu_concat_suffix_temp(ctx, pkg, "_CONFIG");
    String_View dir_key = svu_concat_suffix_temp(ctx, pkg, "_DIR");

    SV_List provider_args = NULL;
    if (!eval_sv_arr_push_temp(ctx, &provider_args, nob_sv_from_cstr("FIND_PACKAGE"))) return false;
    for (size_t i = 0; i < arena_arr_len(*resolved_args); i++) {
    ...
    if (eval_var_defined_visible(ctx, found_key) && eval_truthy(ctx, eval_var_get_visible(ctx, found_key))) {
        *out_found = true;
        *out_found_path = eval_var_get_visible(ctx, config_key);
...
static void find_package_emit_result(EvalExecContext *ctx,
    ...
    if (!found && opt->required) {
        EVAL_NODE_ORIGIN_DIAG_EMIT_SEV(ctx, node, o, EV_DIAG_ERROR, EVAL_DIAG_NOT_FOUND, "dispatcher", nob_sv_from_cstr("Required package not found"), opt->pkg);
        eval_request_stop_on_error(ctx);
    } else if (!found && !opt->quiet) {
        EVAL_NODE_ORIGIN_DIAG_EMIT_SEV(ctx, node, o, EV_DIAG_WARNING, EVAL_DIAG_NOT_FOUND, "dispatcher", nob_sv_from_cstr("Package not found"), opt->pkg);
    }
    if (!eval_should_stop(ctx)) {
...
Eval_Result eval_handle_find_package(EvalExecContext *ctx, const Node *node) {
    ...
    if (arena_arr_len(a) < 1) {
        EVAL_NODE_ORIGIN_DIAG_EMIT_SEV(ctx, node, o, EV_DIAG_ERROR, EVAL_DIAG_MISSING_REQUIRED, "dispatcher", nob_sv_from_cstr("find_package() missing package name"), nob_sv_from_cstr("Usage: find_package(<Pkg> [REQUIRED] [MODULE|CONFIG])"));
