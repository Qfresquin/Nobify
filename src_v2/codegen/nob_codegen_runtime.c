...
bool cg_emit_support_helpers(CG_Context *ctx, Nob_String_Builder *out) {
    ...
    if (ctx->helper_bits & CG_HELPER_RUN_CMD) {
        nob_sb_append_cstr(out,
            "static bool __attribute__((unused)) run_cmd_in_dir(const char *working_dir, Nob_Cmd *cmd) {\n"
            ...
            "    return copy;\n"
            "}\n\n"
            "static bool run_cmd_capture_status_in_dir(const char *working_dir,\n"
            "                                          Nob_Cmd *cmd,\n"
            "                                          char **out_stdout,\n"
            "                                          char **out_stderr,\n"
            "                                          int *out_exit_code);\n\n"
            "static bool run_cmd_capture_status_in_dir_timeout(const char *working_dir,\n"
            "                                                  Nob_Cmd *cmd,\n"
            "                                                  char **out_stdout,\n"
            "                                                  char **out_stderr,\n"
            "                                                  int *out_exit_code,\n"
            ...
            "                                   char **out_stderr) {\n"
            "    int exit_code = 1;\n"
            "    return run_cmd_capture_status_in_dir(working_dir, cmd, out_stdout, out_stderr, &exit_code) && exit_code == 0;\n"
            "}\n\n"
            "static bool run_cmd_capture_status_in_dir(const char *working_dir,\n"
            "                                          Nob_Cmd *cmd,\n"
            "                                          char **out_stdout,\n"
            "                                          char **out_stderr,\n"
            "                                          int *out_exit_code) {\n"
            "    return run_cmd_capture_status_in_dir_timeout(working_dir, cmd, out_stdout, out_stderr, out_exit_code, -1);\n"
            "}\n\n"
            "static bool run_cmd_capture_status_in_dir_timeout(const char *working_dir,\n"
            "                                                  Nob_Cmd *cmd,\n"
            "                                                  char **out_stdout,\n"
            ...
            "    fderr = nob_fd_open_for_write(stderr_path);\n"
            "    if (fderr == NOB_INVALID_FD) { nob_fd_close(fdout); return false; }\n"
            "    if (working_dir && working_dir[0] != '\\0') {\n"
            "        saved_dir = launch_dir;\n"
            "        if (!nob_set_current_dir(working_dir)) { nob_fd_close(fdout); nob_fd_close(fderr); return false; }\n"
            "    }\n"
            "    proc = nob__cmd_start_process(*cmd, NULL, &fdout, &fderr);\n"
            "    nob_fd_close(fdout);\n"
            "    nob_fd_close(fderr);\n"
            "    if (proc == NOB_INVALID_PROC) {\n"
            "        if (working_dir && working_dir[0] != '\\0') (void)nob_set_current_dir(saved_dir);\n"
            "        return false;\n"
            "    }\n"
            "#if defined(_WIN32)\n"
            "    {\n"
            "        DWORD status = 1;\n"
            "        DWORD wait_ms = timeout_seconds >= 0 ? (DWORD)timeout_seconds * 1000u : INFINITE;\n"
            "        DWORD wait_status = WaitForSingleObject(proc, wait_ms);\n"
            "        if (wait_status == WAIT_TIMEOUT) {\n"
            "            TerminateProcess(proc, 124u);\n"
            "            WaitForSingleObject(proc, INFINITE);\n"
            "            status = 124u;\n"
            "        } else if (!GetExitCodeProcess(proc, &status)) {\n"
            "            status = 1;\n"
            "        }\n"
            "        CloseHandle(proc);\n"
            ...
            "    }\n"
            "#else\n"
            "    time_t deadline = timeout_seconds >= 0 ? time(NULL) + timeout_seconds : (time_t)0;\n"
            "    for (;;) {\n"
            "        int wstatus = 0;\n"
            "        pid_t waited = waitpid(proc, &wstatus, timeout_seconds >= 0 ? WNOHANG : 0);\n"
            "        if (waited < 0) {\n"
            ...
            "        }\n"
            "        if (waited == 0) {\n"
            "            if (timeout_seconds >= 0 && time(NULL) >= deadline) {\n"
            "                kill(proc, SIGKILL);\n"
            "                while (waitpid(proc, &wstatus, 0) < 0 && errno == EINTR) {}\n"
            "                exit_code = 124;\n"
            "                break;\n"
            "            }\n"
            "            usleep(10000u);\n"
