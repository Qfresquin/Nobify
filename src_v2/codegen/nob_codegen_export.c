...
static bool cg_export_emit_build_tree_target_properties(CG_Context *ctx,
    ...
    if (!cg_export_collect_build_interface_includes(ctx, export_id, target_id, config, &include_items) ||
    ...
    *out = nob_sv_from_cstr("");
    ...
    for (size_t i = 0; i < targets.count; ++i) {
        ...
        if (!cg_target_exported_name(ctx, target_id, export_namespace, &exported_name) ||
            !cg_export_emit_build_tree_target_properties(ctx,
                                                         export_id,
                                                         target_id,
                                                         exported_name,
                                                         export_namespace,
...
static bool cg_emit_package_registry_helpers(Nob_String_Builder *out) {
    ...
    nob_sb_append_cstr(out,
        "static unsigned int export_registry_md5_load_be32(const unsigned char *p) {\n"
