...
static const EGD_Subcommand_Inventory s_egd_subcommand_inventory[] = {
    ...
    {"ctest_*", "ctest_empty_binary_directory|ctest_start|ctest_configure|ctest_build|ctest_test|ctest_sleep", EGD_PACK_CTEST, "ctest_local_dashboard_parity_surface", EGD_CLASS_PARITY_PASS, EGD_PHASE_TEST | EGD_PHASE_HOST_ONLY, "build-model.replay.test-driver", "local dashboard ctest steps are part of the closed local CTest replay row and replay through the generated backend test-driver runtime", NULL, "workload.codegen.local-ctest"},
    {"ctest_*", "ctest_coverage|ctest_memcheck", EGD_PACK_CTEST, "ctest_local_coverage_memcheck_parity_surface", EGD_CLASS_PARITY_PASS, EGD_PHASE_TEST | EGD_PHASE_HOST_ONLY, "build-model.replay.test-driver", "local coverage and memcheck dashboard steps are part of the closed local CTest replay row and replay through the generated backend", NULL, "workload.codegen.local-ctest-extended"},
    {"ctest_*", "ctest_submit|ctest_upload|ctest_run_script|ctest_read_custom_files|ctest_update", EGD_PACK_CTEST, "ctest_local_dashboard_surface", EGD_CLASS_BACKEND_REJECT, EGD_PHASE_TEST | EGD_PHASE_HOST_ONLY, "build-model.replay.test-driver", "networked, VCS, and child-script ctest variants remain explicit backend rejects outside the closed local CTest replay row", "epic-c.ctest-extended", NULL},
    {"FetchContent_*", "SOURCE_DIR|LOCAL_ARCHIVE", EGD_PACK_FETCHCONTENT, "fetchcontent_local_materialization_surface", EGD_CLASS_PARITY_PASS, EGD_PHASE_CONFIGURE | EGD_PHASE_BUILD, "build-model.replay.dependency-materialization", "local deterministic FetchContent source-dir and archive materialization are the backend parity proof for the closed dependency-materialization row", NULL, "workload.codegen.fetchcontent-local"},
...
static const EGD_Case_Def s_egd_cases[] = {
    ...
    {"fetchcontent_local_materialization_surface", EGD_PACK_FETCHCONTENT, "FetchContent_MakeAvailable", "local SOURCE_DIR + local archive materialization", EGD_CLASS_PARITY_PASS, EGD_PARITY_BUILD_TREE, EGD_OUTCOME_SUCCESS, EGD_PHASE_CONFIGURE | EGD_PHASE_BUILD, EGD_TOOL_CMAKE | EGD_TOOL_TAR, "build-model.replay.dependency-materialization", "closure-matrix parity owner for local deterministic FetchContent dependency materialization", NULL, "workload.codegen.fetchcontent-local", s_egd_fetchcontent_local_outputs, NOB_ARRAY_LEN(s_egd_fetchcontent_local_outputs), s_egd_fetchcontent_local_manifests, NOB_ARRAY_LEN(s_egd_fetchcontent_local_manifests), NULL},
    {"ctest_local_coverage_memcheck_parity_surface", EGD_PACK_CTEST, "ctest_coverage|ctest_memcheck", "local CTest replay closure", EGD_CLASS_PARITY_PASS, EGD_PARITY_BUILD_TREE, EGD_OUTCOME_SUCCESS, EGD_PHASE_TEST | EGD_PHASE_HOST_ONLY, EGD_TOOL_CMAKE, "build-model.replay.test-driver", "focused closure-matrix parity for local-only CTest coverage and memcheck staging through the generated backend", NULL, "workload.codegen.local-ctest-extended", s_egd_ctest_extended_outputs, NOB_ARRAY_LEN(s_egd_ctest_extended_outputs), s_egd_ctest_extended_manifests, NOB_ARRAY_LEN(s_egd_ctest_extended_manifests), NULL},
